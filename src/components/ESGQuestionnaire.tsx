...
import { ArrowLeft, ArrowRight, Building2, Leaf, Users, ShieldCheck } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { questions, sectors, employeeCounts, categoryDescriptions, type ScanAnswers, type Question } from "@/lib/esgQuestions";
...
const steps = ["company", "environment", "social", "governance"] as const;
...
export default function ESGQuestionnaire({ onComplete, onBack }: Props) {
  ...
  const currentQuestions = isCompanyStep ? [] : questions.filter((q) => q.category === currentStep);

  const canProceed = () => {
    if (isCompanyStep) return companyName.trim() && sector && employeeCount;
  ...
  const handleNext = () => {
    if (step < steps.length - 1) {
      ...
      onComplete({ companyName, sector, employeeCount, answers });
    }
  };
    ...
    <div className="min-h-screen flex flex-col">
                ...
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <div className="h-10 w-10 rounded-xl bg-secondary/10 flex items-center justify-center">
                      {(() => { const Icon = categoryIcons[currentStep]; return <Icon className="h-5 w-5 text-secondary" />; })()}
                    </div>
                    <h2 className="font-heading text-2xl font-bold text-primary capitalize">{currentStep}</h2>
                  </div>
                  <p className="text-muted-foreground mb-8">{categoryDescriptions[currentStep]}</p>
      ...
      <div className="px-6 py-5 border-t border-border/50 flex justify-between">
        ...
        <Button
          onClick={handleNext}
          disabled={!canProceed()}
          className="bg-secondary hover:bg-secondary/90 text-secondary-foreground font-semibold px-6 rounded-xl disabled:opacity-40"
        >
    ...
    <div>
      <div className="mb-3">
        ...
        {question.description && (
          <p className="text-sm text-muted-foreground mt-1 ml-5">{question.description}</p>
        )}
      </div>
